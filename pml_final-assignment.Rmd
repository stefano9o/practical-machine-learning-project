...
test.sel <- test[c(outcome,features.accel,features.gyros,features.roll,features.pitch,features.yam)]
```

```{r}
factorIndexes <- sapply(names(train.sel),function(featureName){
        (class(train.sel[,featureName]) == "factor") && featureName != "class"
})

length(which(factorIndexes))
train.sel[,factorIndexes] <- sapply(train.sel[,factorIndexes],as.numeric)
