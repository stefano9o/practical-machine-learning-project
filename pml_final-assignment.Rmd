...
```{r}
test.sel$X <- NULL
test.sel$user_name <- NULL
test.sel$raw_timestamp_part_1 <- NULL
test.sel$raw_timestamp_part_2 <- NULL
test.sel$cvtd_timestamp <- NULL
test.sel$new_window <- NULL
test.sel$num_window <- NULL
```

```{r}
rawFeatures <- grep(pattern = "^(accel|gyros|magnet)_.*_(x|y|z)$",x = names(train.sel))
rawFeatures <- c(rawFeatures,grep(pattern = "^(roll|pitch|yaw)_.*",x = names(train.sel)))
rawFeatures <- c(rawFeatures,grep(pattern = "^total_accel.*",x = names(train.sel)))

train.sel[,rawFeatures] <- NULL
test.sel[,rawFeatures] <- NULL
```
