...
First of all, we are going to remove from the training dataset all the variable with a correletion higher then `0.75`: 
...
The most important parameters to be tuned for the `rf` method are: the number of tree to grow (`ntree`), and the number of predictors to be taken into account as potential splitting variable at each split (`mtry`). by deafult, the `train` function keeps fixed tthe first parameter (`ntree=500`), while it varies the second; in our case it assuemes three diferent values (`mtry=2`,`mtry=17`, and `mtry=34`). From the results above, it turns out that `mtry = 2` produce the best results.
